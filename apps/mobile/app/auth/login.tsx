...
import {
  ...
  ScrollView, TouchableOpacity, BackHandler, Animated,
} from "react-native";
import { useForm, Controller } from "react-hook-form";
import { Link, useRouter, useLocalSearchParams } from "expo-router";
import { Leaf, ArrowLeft, CheckCircle2, Eye, EyeOff } from "lucide-react-native";
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={styles.card}>
            <View style={styles.header}>
              <View style={styles.iconContainer}><Leaf size={28} color="#16a34a" /></View>
              <Text style={styles.title}>Chào mừng trở lại</Text>
              <Text style={styles.subtitle}>Đăng nhập để tiếp tục quản lý vườn cây của bạn</Text>
            </View>

            {registered === "true" && (
              <Animated.View style={[styles.successAlert, { opacity: fadeAnim, transform: [{ translateY: translateYAnim }] }]}>
                <CheckCircle2 size={20} color="#15803d" style={{ marginTop: 2 }} />
                <View style={styles.successTextContainer}>
                  <Text style={styles.successTitle}>Đăng ký thành công!</Text>
                  <Text style={styles.successDesc}>Vui lòng đăng nhập bằng email và mật khẩu vừa tạo.</Text>
                </View>
              </Animated.View>
            )}
            ...
            <View style={styles.form}>
              ...
              <View style={styles.passwordHeader}>
                ...
                <Link href="/auth/forgot-password" style={styles.forgotLink}>Quên mật khẩu?</Link>
              </View>
                ...
                render={({ field: { onChange, onBlur, value }, fieldState: { error } }) => (
                  <View style={styles.inputGroup}>
                    <View style={styles.passwordWrapper}>
                      <Input placeholder="••••••••" secureTextEntry={!showPassword}
                        onBlur={onBlur} onChangeText={onChange} value={value} error={error?.message} />
              ...
              <Button title="Đăng nhập" variant="primary" size="lg"
                isLoading={isSubmitting} onPress={handleSubmit(onSubmit)} style={styles.submitButton} />

              <View style={styles.footer}>
                <Text style={styles.footerText}>Chưa có tài khoản? </Text>
                <Link href="/auth/register" style={styles.registerLink}>Đăng ký ngay</Link>
              </View>
            </View>
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  scrollContent: { flexGrow: 1, justifyContent: "center", paddingBottom: 40, paddingHorizontal: 16 },
  ...
  successAlert: { flexDirection: "row", backgroundColor: "#f0fdf4", borderWidth: 1, borderColor: "#bbf7d0", borderRadius: 12, padding: 16, marginBottom: 24, alignItems: "flex-start" },
  ...
  passwordHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 6, marginTop: 4 },
