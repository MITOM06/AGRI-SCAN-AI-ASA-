...
class ViT_MoE(nn.Module):
    def __init__(self, num_classes, num_experts=3, backbone_name='vit_base_patch16_224', pretrained=False, image_size=224):
        ...
        self.experts = nn.ModuleList([
            Expert(self.feature_dim, hidden=min(512, max(128, self.feature_dim // 2)), num_classes=num_classes)
...
# ═══════════════════════════════════════════════════════════════════
# BUG #4 FIX: Đường dẫn sai "model" → phải là "models"
#
# Theo project_tree.txt:
#   apps/ai-service/models/vit_moe_best.pth       ← thư mục "models" (có chữ s)
#   apps/ai-service/models/class_to_idx.json
#
# Code cũ: Path(__file__).resolve().parents[1] / "model"   → KHÔNG TỒN TẠI
# Code fix: Path(__file__).resolve().parents[1] / "models" → đúng
# ═══════════════════════════════════════════════════════════════════
...
def load_labels():
    ...
    try:
        if LABEL_JSON_PATH.exists():
            ...
            print(f"[INFO] Loaded {len(idx_to_class)} labels from {LABEL_JSON_PATH}")
        else:
...
def load_yolo(model_path: Optional[str] = None):
    ...
    print(f"[INFO] Initializing ViT_MoE with {num_classes} classes from {mp}...")
    try:
    ...
    model.device = device
    yolo_model = model
    print(f"[INFO] Model running on: {device}")
    return yolo_model
