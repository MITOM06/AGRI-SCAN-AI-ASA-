...
export const authApi = {
    ...
    resetPassword: async (data: any) => {
        const response = await axiosClient.post(API_ENDPOINTS.AUTH.RESET_PASSWORD, data);
        return response.data;
    },

    getProfile: async () => (await axiosClient.get(API_ENDPOINTS.AUTH.PROFILE)).data,
    ...
    setPassword: async (newPassword: string) => {
        const response = await axiosClient.post(API_ENDPOINTS.AUTH.SET_PASSWORD, { newPassword });
        return response.data;
    },
};